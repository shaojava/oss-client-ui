 ...
 * 文件列表
 */
    .controller('FileListCtrl', ['OSSUploadQueue', '$scope', '$routeParams', 'localStorageService', 'OSSApi', 'buckets', '$rootScope', 'OSSObject', 'OSSMenu', 'Bucket', '$route', '$location', 'OSSLocation', 'usSpinnerService', '$filter', 'OSSException', '$timeout',
        function (OSSUploadQueue, $scope, $routeParams, localStorageService, OSSApi, buckets, $rootScope, OSSObject, OSSMenu, Bucket, $route, $location, OSSLocation, usSpinnerService, $filter, OSSException, $timeout) {
            ...
            //上传配置项
            $scope.webUploaderOptions = {
                auto:true
            };

            $scope.webUploaderCallbacks = {
                fileQueued: function (file) {
                    console.log('fileQueued', arguments);
                    OSSUploadQueue.add({
                        pathhash: file.id,
                        bucket: $scope.bucket['Name'],
                        object: [$scope.objectPrefix , file.name].join(''),
                        fullpath: file.name,
                        offset: 0,
                        filesize: file.size,
                        status: 2
                ...
                uploadStart:function(file){
                    var item = OSSUploadQueue.get(file.id);
                    OSSUploadQueue.update(item,{
                        status:1
                    });
                },
